...
import { Search, Filter, MoreHorizontal, Eye, Edit, Trash2, Power, PowerOff, Upload, Download, Heart, Copy, History, Star, AlertTriangle } from 'lucide-react';
...
import { CopyFormatsDialog } from '@/components/CopyFormatsDialog';
...
const SecretsList = () => {
  ...
  const filteredSecrets = useMemo(() => {
    return secrets.filter(secret => {
      const matchesSearch = 
        ...
        secret.meta.description.toLowerCase().includes(searchQuery.toLowerCase()) ||
        secret.meta.tags.some(tag => tag.toLowerCase().includes(searchQuery.toLowerCase()));
      
      const matchesEnvironment = environmentFilter === 'all' || secret.environment === environmentFilter;
      const matchesProject = projectFilter === 'all' || secret.project.name === projectFilter;
      ...
      return matchesSearch && matchesEnvironment && matchesProject && matchesStatus && matchesFavorite;
    }).sort((a, b) => {
  ...
  }, [secrets, searchQuery, environmentFilter, projectFilter, statusFilter, favoriteFilter]);
  ...
  const handleToggleFavorite = (secret: Secret) => {
    ...
    toast({
      title: secret.meta.isFavorite ? 'Removed from favorites' : 'Added to favorites',
      description: `${secret.name} has been ${secret.meta.isFavorite ? 'removed from' : 'added to'} favorites.`,
    });
  };
  ...
  const allVisibleSelected = filteredSecrets.length > 0 && filteredSecrets.every(s => selectedSecrets.includes(s.id));
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Manage and search through all your environment variables and secrets
          </p>
        </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex space-x-2">
              {(searchQuery || environmentFilter !== 'all' || projectFilter !== 'all' || statusFilter !== 'all' || favoriteFilter !== 'all') && (
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => {
        ...
        <CardContent className="p-0">
          <Table>
            ...
            <TableBody>
              {filteredSecrets.map((secret) => (
                <TableRow key={secret.id} className={selectedSecrets.includes(secret.id) ? 'bg-muted/50' : ''}>
                  <TableCell>
                    <Checkbox
                  ...
                  <TableCell className="font-mono text-sm">{secret.identifier}</TableCell>
                  <TableCell>
                    <div>
                      <div className="font-medium">{secret.project.name}</div>
                      {secret.project.module && (
                        <div className="text-xs text-muted-foreground">{secret.project.module}</div>
                      )}
                    </div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem onClick={() => handleShowCopyFormats(secret)}>
                          <Copy className="h-4 w-4 mr-2" />
                          Copy Formats
                        </DropdownMenuItem>
                        <DropdownMenuSeparator />
                        <DropdownMenuItem onClick={() => handleToggleFavorite(secret)}>
                          <Heart className={`h-4 w-4 mr-2 ${secret.meta.isFavorite ? 'text-red-500 fill-current' : ''}`} />
                          {secret.meta.isFavorite ? 'Remove from Favorites' : 'Add to Favorites'}
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => handleToggleStatus(secret)}>
                        ...
                        {secret.history && secret.history.length > 1 && (
                          <DropdownMenuItem onClick={() => handleShowHistory(secret)}>
          ...
          {filteredSecrets.length === 0 && (
            <div className="text-center py-12">
              <p className="text-muted-foreground">No secrets found matching your criteria.</p>
            </div>
          )}
        </CardContent>
      </Card>
