...
export const SecretsProvider: React.FC<SecretsProviderProps> = ({ children }) => {
  ...
  // Derived: recentlyAccessed unchanged...
  const recentlyAccessed = useMemo(() => {
    return secrets
      .filter((s) => s.meta.lastAccessed)
      .sort(
        (a, b) =>
          (b.meta.lastAccessed?.getTime() || 0) -
          (a.meta.lastAccessed?.getTime() || 0)
      )
      .slice(0, 5);
  }, [secrets]);

  // NEW: allTags derived
  const allTags = useMemo(() => collectAllTags(secrets), [secrets]);
      ...
      prev.map((secret) => {
        if (secret.id === id) {
          ...
          const hasValueChanged =
            updatedSecret.value && updatedSecret.value !== secret.value;
    ...
    setSecrets((prev) =>
      prev.map((secret) => {
        if (secret.id === id && secret.history) {
          const historyEntry = secret.history.find(
            (h) => h.version === version
          );
          if (historyEntry) {
            const newVersion = (secret.meta.version || 1) + 1;
  ...
  const exportToJson = (secretIds?: string[]): Secret[] => {
    return secrets.filter(
      (secret) => !secretIds || secretIds.includes(secret.id)
    );
  };
  ...
  const updateSecretTags = useCallback((id: string, tags: string[]) => {
    setSecrets((prev) =>
      prev.map((secret) =>
        secret.id === id
          ? {
